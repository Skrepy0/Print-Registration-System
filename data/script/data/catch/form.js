...
function editData(key) {
  showEditDataModal();
  const record = data.catchTeacherList[Object.keys(data.catchTeacherList).find(k => k === key)];
  if (!record) {
  ...
  // 其他字段切换
  document.getElementById('edit-grade-data').addEventListener('change', function () {
    document.getElementById('edit-grade-data-other-box').classList.toggle('hidden', this.value !== '其他');
  });
  document.getElementById('edit-subject-data').addEventListener('change', function () {
    document.getElementById('edit-subject-data-other-box').classList.toggle('hidden', this.value !== '其他');
  });
  document.getElementById('edit-submitter-data').addEventListener('change', function () {
    document.getElementById('edit-submitter-data-other-box').classList.toggle('hidden', this.value !== '其他');
  })
  document.getElementById('edit-submitter-data').addEventListener('change', function () {
    ...
    if (submitter && submitter.value !== '其他') {
      if (getAutoDataBySubmitter[submitter.value]) {
        document.getElementById('edit-subject-data').value = getAutoDataBySubmitter[submitter.value][1];
        document.getElementById('edit-grade-data').value = getAutoDataBySubmitter[submitter.value][0];
      }
    }
  })

  document.getElementById('cancel-edit-data').addEventListener('click', closeEditDataPage);
  ...
  const handleEditData = (e) => {
    e.preventDefault();
    const finalGrade = getValue(document.getElementById('edit-grade-data'), document.getElementById('edit-grade-data-other'));
    const finalSubject = getValue(document.getElementById('edit-subject-data'), document.getElementById('edit-subject-data-other'));
    const finalSubmitter = getValue(document.getElementById('edit-submitter-data'), document.getElementById('edit-submitter-data-other'));
...
function getFilterData() {
  ...
  Object.entries(data.catchTeacherList).forEach(([teacher, [subject, grade]]) => {
    if (teacher.toLowerCase().includes(term) ||
...
export function renderData() {
  ...
  if(!keys.length){
    constants.editCatchTBody.innerHTML = '<tr><td colspan="8" class="px-3 py-8 text-center text-neutral/60"><i class="fa fa-inbox mr-2"></i>暂无记录</td></tr>';
    return;
  }
  keys.forEach(key => {
