...
export function handleFormSubmit(e) {
  ...
  const subject = getFinalValue(
    constants.subjectSelect,
    constants.subjectOtherInput
  )
  const submitter = getSubmitterFinalValue(
    constants.submitterSelect,
    constants.submitterOtherInput
  )
  const paperSize = getFinalValue(
    constants.paperSizeSelect,
    constants.paperSizeOtherInput
  )
  const expenseType = getFinalValue(
    constants.expenseTypeSelect,
    constants.expenseTypeOtherInput
  )
  saveData()
  if (!grade || grade === '请选择年级' || grade === '其他')
    return showToast('请选择年级', 'warning')
  if (!subject || subject === '其他')
    return showToast('请选择或输入学科', 'warning')
  if (!submitter || submitter === '其他')
    return showToast('请输入有效的送印人', 'warning')
...
export function handleSort(f) {
  ...
  document
    .querySelectorAll('.sortable i')
    .forEach((i) => (i.className = 'fa fa-sort ml-1'))
...
export function handleSelectAll() {
  document
    .querySelectorAll('.record-select')
    .forEach((cb) => (cb.checked = constants.selectAllCheckbox.checked))
}
