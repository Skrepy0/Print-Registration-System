...
export function renderRecords() {
  ...
  if (state.sortField) {
    filtered.sort((a, b) => {
      ...
      if (va == null || va === '')
        va = state.sortDirection === 'asc' ? '\uffff' : ''
      if (vb == null || vb === '')
        vb = state.sortDirection === 'asc' ? '\uffff' : ''
...
export function editRecord(id) {
  ...
  const isCustomPaperSize = !constants.PAPER_SIZE_OPTIONS.includes(
    record.paperSize
  )
  const isCustomExpenseType = !state.expense_type_options.includes(
    record.expenseType
  )
  ...
  document.getElementById('edit-grade').addEventListener('change', function () {
    document
      .getElementById('edit-grade-other-box')
  ...
  document
    ...
    .addEventListener('change', function () {
      document
        .getElementById('edit-subject-other-box')
        .classList.toggle('hidden', this.value !== '其他')
    })
  document
    .getElementById('edit-submitter')
    .addEventListener('change', function () {
      document
        .getElementById('edit-submitter-other-box')
        .classList.toggle('hidden', this.value !== '其他')
    })
  document
    .getElementById('edit-submitter')
  ...
  document
    ...
    .addEventListener('change', function () {
      document
        .getElementById('edit-expense-other-box')
        .classList.toggle('hidden', this.value !== '其他')
    })
  ...
  const handleEditSubmit = (e) => {
    e.preventDefault()
    const finalGrade = getFinalValue(
      document.getElementById('edit-grade'),
      document.getElementById('edit-grade-other')
    )
    const finalSubject = getFinalValue(
      document.getElementById('edit-subject'),
      document.getElementById('edit-subject-other')
    )
    const finalSubmitter = getFinalValue(
      document.getElementById('edit-submitter'),
    ...
    const finalExpenseType = getFinalValue(
      ...
      document.getElementById('edit-expense-other')
    )
    if (!document.getElementById('edit-date').value)
      return showToast('请选择日期', 'warning')
...
export function saveRecords() {
  state.records = state.records.map(
    ({ unitPrice, totalAmount, ...keep }) => keep
  )
...
export function updateSelectAllStatus() {
  ...
  constants.selectAllCheckbox.checked =
