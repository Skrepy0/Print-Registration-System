...
export function exportSelectedRecords() {
  ...
  exportRecordsToExcel(
    state.records.filter((r) => ids.includes(r.id)),
    '印刷记录_选中'
  )
}
...
export function backupData() {
  ...
  const cleanForBackup = state.records.map(
    ({ unitPrice, totalAmount, ...keep }) => keep
  )
  const blob = new Blob(
    [JSON.stringify({ records: cleanForBackup, time: new Date() })],
...
export function backupSubmitterData() {
  if (
    ...
    Object.keys(data.catchTeacherList).length === 0
  ) {
  ...
  reader.onload = (ev) => {
    try {
      ...
      if (!data.records || !Array.isArray(data.records))
        throw new Error('无效备份')
      const cleanedIncoming = data.records.map(
        ({ unitPrice, totalAmount, ...rest }) => rest
      )
      const isValid = cleanedIncoming.every(
        (r) => r.id && r.date && r.subject && r.submitter
      )
      if (!isValid) {
      ...
      function handleData(mode) {
        if (mode) {
          ...
          const newRecords = cleanedIncoming.filter(
            (r) => !existingIds.has(r.id)
          )
          const updatedRecords = cleanedIncoming.filter((r) =>
            existingIds.has(r.id)
          )
          updatedRecords.forEach((backupRecord) => {
            const index = state.records.findIndex(
              (r) => r.id === backupRecord.id
            )
